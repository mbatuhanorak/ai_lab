...
//------------------------------------------------------------------------------
// Testbench: tb_axi_master_bug1
//
// Verification testbench for axi_master_bug1 DUT.
// Uses AXI VIP (slave memory model) and AXI Protocol Checker.
//------------------------------------------------------------------------------

`timescale 1ns / 1ps

module tb_axi_master_bug1();
    ...
    bit violation_seen = 0;
    int violation_cycle_count = 0;
    localparam int VIOLATION_GRACE_CYCLES = 20;
    ...
    always @(posedge aclk) begin
        ...
        if (violation_seen) begin
            violation_cycle_count++;
            if (violation_cycle_count == VIOLATION_GRACE_CYCLES)
                $fatal(1, "FAIL: AXI protocol violations detected (pc_status = 0x%040h). Stopping after %0d grace cycles.",
                       pc_status, VIOLATION_GRACE_CYCLES);
        end
    end
    ...
    initial begin
        ...
        $display("");
        $display("========================================================");
        $display("  DUT VARIANT #1");
        $display("  DUT: axi_master_bug1");
        ...
        $display("========================================================");
        if (done) begin
            $display("  DUT completed.");
            if (error)
                $display("  DUT reported error flag.");
        end else begin
            $display("  DUT did NOT complete — simulation timed out.");
        end
        if (violation_seen || pc_asserted)
        ...
        if (violation_seen || pc_asserted)
            $fatal(1, "FAIL: AXI protocol violations detected (pc_status = 0x%040h).", pc_status);
        if (error)
            $fatal(1, "FAIL: DUT reported error flag.");
        if (!done)
            $fatal(1, "FAIL: DUT did not complete — timeout.");

        $display("PASS: Test completed without errors.");
        $finish;
    end
