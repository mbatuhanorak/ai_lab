...
module axi_master_bug4 #(
    ...
    // AWVALID control
    logic awvalid_latch = 1'b0;

    always_comb begin
        if (state_q == ST_WR_ADDR)
            awvalid_latch = 1'b1;
    end

    assign m_axi_awvalid = awvalid_latch;
    ...
    always_comb begin
        ...
        if (state_q == ST_WR_ADDR) begin
            m_axi_awid = cur_id; m_axi_awaddr = cur_addr; m_axi_awlen = cur_len;
        end
    end
    ...
    always_comb begin
        ...
        if (state_q == ST_WR_DATA) begin
            m_axi_wvalid = 1'b1; m_axi_wdata = cur_beat_wdata;
            m_axi_wstrb = {STRB_WIDTH{1'b1}}; m_axi_wlast = (beat_q == cur_len);
        end
    end
